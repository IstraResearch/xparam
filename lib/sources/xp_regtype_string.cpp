...
class string_output_functor {
  public:
    static void output(std::ostream& os, const std::string& s) {
      os << '\"';

	  std::string::const_iterator i;
	  for (i=s.begin(); i!=s.end(); ++i)
		  CharBodyOutput::output(os,*i,true);
	  os << '\"';
    }
};
...
PARAM_BEGIN_REG
  ...
  param_direct_output<std::string,string_output_functor>();
  param_vector<std::string>();
PARAM_END_REG
