...
struct everything {
  public:
    ...
    everything(long double ld) : msg("From long double") {}
    ...
    everything(const vector<long double>& vld) : msg("From vector of long double") {}
...
param_ctor(TypeTag<everything>(),ByVal<long double>("ld"));
...
param_ctor(TypeTag<everything>(),ConstRef<vector<long double> >("vld"));
...
int main(void)
{
  ...
  cout << "Testing conversions..." << endl;
  {
    ...
    ParamSet ps;
    ps << ioParamVar(vll,"vll")
       << ioParamVar(vull,"vull")
       << ioParamVar(vld,"vld");

    const char* argv[]={"progname",
            "vll=[1.0l] vull=[1.0l] vld=ulonglong(1),longlong(1)"};
